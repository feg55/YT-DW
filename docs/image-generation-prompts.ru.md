...
из него создаётся многоразмерный `openmediadl.ico` для Windows. Баннер README, превью для соцсетей,
заглушки обложек и дополнительные изображения не нужны.

## Куда сохранить

- `src/openmediadl/resources/icons/openmediadl.png` — исходная иконка для интерфейса.
- `src/openmediadl/resources/icons/openmediadl.ico` — производная иконка внутри Windows EXE.

Рекомендуемый исходник: квадратный PNG 512×512 или больше, sRGB. Для ICO нужны слои 16, 24, 32,
48, 64, 128 и 256 px. В готовой сборке оба файла встроены в `dist\OpenMediaDL.exe`; хранить их рядом
с EXE не требуется.

## Промпт на случай будущей перегенерации

```text
Create one original square desktop application icon for OpenMediaDL. Show a bold downward transfer
